        ...
        if (trimmed.startsWith('{') || trimmed.startsWith('[')) {
            ...
            try {
                ...
                if (jsonData && typeof jsonData === 'object') {
                    ...
                    // Build stats array - only include stats that are explicitly enabled in config
                    // This prevents ghost/phantom stats from leaking in via extensionSettings.userStats
                    const statsArray = [];
...
export function renderUserStats() {
    ...
    for (const stat of enabledStats) {
        ...
        html += `
            <div class="rpg-stat-row"${stat.description ? ` data-stat-description="${stat.description.replace(/"/g, '&quot;')}"` : ''}>
