...
export class EncounterModal {
    ...
    createModal() {
        ...
        // Inject custom stat bar styles if not already present
        if (!document.getElementById('rpg-encounter-custom-stats-style')) {
            const style = document.createElement('style');
            style.id = 'rpg-encounter-custom-stats-style';
            style.textContent = `
                .rpg-encounter-custom-stat-row {
                    display: flex;
                    align-items: center;
                    gap: 4px;
                    margin-top: 3px;
                }
                .rpg-encounter-custom-stat-label {
                    font-size: 0.65em;
                    opacity: 0.75;
                    min-width: 44px;
                    white-space: nowrap;
                ...
                .rpg-encounter-custom-stat-fill {
                    height: 100%;
                    border-radius: 3px;
                    transition: width 0.4s ease, background 0.4s ease;
                }
                .rpg-encounter-custom-stat-value {
                    font-size: 0.62em;
                    opacity: 0.7;
                    min-width: 32px;
                    text-align: right;
            ...
            return `
                <div class="rpg-encounter-card ${isDead ? 'rpg-encounter-dead' : ''}" data-enemy-index="${index}">
                    ...
                    <div class="rpg-encounter-card-info">
                        ...
                        ${enemy.customStats && enemy.customStats.length > 0 ? enemy.customStats.map(s => {
                            const pct = s.maxValue > 0 ? Math.round((s.value / s.maxValue) * 100) : Math.round(s.value);
                            const barColor = pct <= 15 ? '#e94560' : pct <= 30 ? '#e0a030' : '#4a9eff';
                            return `
                        <div class="rpg-encounter-custom-stat-row" title="${s.name}: ${s.value}/${s.maxValue}">
                            <span class="rpg-encounter-custom-stat-label">${s.name}</span>
                            <div class="rpg-encounter-custom-stat-bar">
                                <div class="rpg-encounter-custom-stat-fill" style="width: ${pct}%; background: ${barColor};"></div>
                            </div>
                            <span class="rpg-encounter-custom-stat-value">${s.value}/${s.maxValue}</span>
                        </div>`;
                        }).join('') : ''}
            ...
            return `
                <div class="rpg-encounter-card ${isDead ? 'rpg-encounter-dead' : ''}" data-party-index="${index}">
                    ...
                    <div class="rpg-encounter-card-info">
                        ...
                        ${member.customStats && member.customStats.length > 0 ? member.customStats.map(s => {
                            const pct = s.maxValue > 0 ? Math.round((s.value / s.maxValue) * 100) : Math.round(s.value);
                            const barColor = pct <= 15 ? '#e94560' : pct <= 30 ? '#e0a030' : '#4a9eff';
                            return `
                        <div class="rpg-encounter-custom-stat-row" title="${s.name}: ${s.value}/${s.maxValue}">
    ...
    updateCombatUI(combatStats) {
        ...
        combatStats.enemies.forEach((enemy, index) => {
            ...
            if (card) {
                ...
                // Update custom stat bars if present
                if (enemy.customStats && enemy.customStats.length > 0) {
                    enemy.customStats.forEach(s => {
                        const row = card.querySelector(`.rpg-encounter-custom-stat-row[title^="${s.name}"]`);
                        if (row) {
                            const pct = s.maxValue > 0 ? Math.round((s.value / s.maxValue) * 100) : Math.round(s.value);
                            const barColor = pct <= 15 ? '#e94560' : pct <= 30 ? '#e0a030' : '#4a9eff';
                            const fill = row.querySelector('.rpg-encounter-custom-stat-fill');
                            const valueSpan = row.querySelector('.rpg-encounter-custom-stat-value');
                            if (fill) { fill.style.width = `${pct}%`; fill.style.background = barColor; }
                            if (valueSpan) valueSpan.textContent = `${s.value}/${s.maxValue}`;
                            row.title = `${s.name}: ${s.value}/${s.maxValue}`;
                        }
                    });
                }
            }
