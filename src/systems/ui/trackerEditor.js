...
function resetToDefaults() {
    extensionSettings.trackerConfig = {
        userStats: {
            customStats: [
                { id: 'health', name: i18n.getTranslation('stats.health') || 'Health', enabled: true, persistInHistory: false, description: '' },
                { id: 'satiety', name: i18n.getTranslation('stats.satiety') || 'Satiety', enabled: true, persistInHistory: false, description: '' },
                { id: 'energy', name: i18n.getTranslation('stats.energy') || 'Energy', enabled: true, persistInHistory: false, description: '' },
                { id: 'hygiene', name: i18n.getTranslation('stats.hygiene') || 'Hygiene', enabled: true, persistInHistory: false, description: '' },
                { id: 'arousal', name: i18n.getTranslation('stats.arousal') || 'Arousal', enabled: true, persistInHistory: false, description: '' }
            ],
    ...
    config.customStats.forEach((stat, index) => {
        ...
        html += `
            <div class="rpg-editor-stat-item" data-index="${index}">
                ...
                <textarea class="rpg-stat-description-input" data-index="${index}" rows="1" placeholder="Description for AI (e.g. Physical health — drops from injuries or exhaustion)">${stat.description || ''}</textarea>
            </div>
        `;
    });
...
function setupUserStatsListeners() {
    ...
    $('.rpg-stat-description-input').off('blur').on('blur', function () {
        const index = $(this).data('index');
        extensionSettings.trackerConfig.userStats.customStats[index].description = $(this).val().trim();
    });
    ...
    charStats.forEach((stat, index) => {
        html += `
            <div class="rpg-editor-field-item" data-index="${index}">
                ...
                <textarea class="rpg-stat-description-input rpg-char-stat-description" data-index="${index}" rows="1" placeholder="Description for AI (e.g. Physical condition of the character)">${stat.description || ''}</textarea>
            </div>
        `;
    });
...
function setupPresentCharactersListeners() {
    ...
    // Update character stat description
    $('.rpg-char-stat-description').off('blur').on('blur', function () {
        const index = $(this).data('index');
        extensionSettings.trackerConfig.presentCharacters.characterStats.customStats[index].description = $(this).val().trim();
    });
}
