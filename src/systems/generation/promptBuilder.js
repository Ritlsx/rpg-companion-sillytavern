            ...
            if (data.stats && Array.isArray(data.stats)) {
                for (const stat of data.stats) {
                    if (stat && stat.value !== undefined) {
                        ...
                        // Look up description from trackerConfig
                        const statConfig = (extensionSettings.trackerConfig?.userStats?.customStats || []).find(s => s.id === statId);
                        const desc = statConfig?.description ? ` — ${statConfig.description}` : '';
                        formatted += `${statName}: ${formatStatValue(stat.value, statId)}${desc}\n`;
                    }
                }
            } else {
            ...
            if (Array.isArray(data)) {
                ...
                for (const char of data) {
                    ...
                    if (char.stats && typeof char.stats === 'object' && !Array.isArray(char.stats)) {
                        const charStatsConfig = extensionSettings.trackerConfig?.presentCharacters?.characterStats?.customStats || [];
                        const statsList = Object.entries(char.stats)
                            .map(([name, val]) => {
                                const statValue = getValue(val);
                                if (!statValue) return null;
                                const statConfig = charStatsConfig.find(s => s.name === name);
                                const desc = statConfig?.description ? ` — ${statConfig.description}` : '';
                                return `${name}: ${statValue}${desc}`;
                            })
                            .filter(s => s)
                            .join(', ');
