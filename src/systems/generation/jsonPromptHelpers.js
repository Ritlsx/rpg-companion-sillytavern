...
export function buildUserStatsJSONInstruction() {
    ...
    // Only emit the stats block if there are enabled stats to show
    if (enabledStats.length > 0) {
        ...
        for (let i = 0; i < enabledStats.length; i++) {
            ...
            const descComment = stat.description ? ` — ${stat.description}` : '';
            if (displayMode === 'number') {
                const maxValue = stat.maxValue || 100;
                instruction += `    {"id": "${stat.id}", "name": "${stat.name}", "value": X}${comma}  // 0 to ${maxValue}${descComment}\n`;
            } else {
                instruction += `    {"id": "${stat.id}", "name": "${stat.name}", "value": X}${comma}  // 0 to 100 (percentage)${descComment}\n`;
            }
        }
...
export function buildCharactersJSONInstruction() {
    ...
    if (enabledCharStats.length > 0) {
        ...
        for (let i = 0; i < enabledCharStats.length; i++) {
            ...
            const charStatDesc = stat.description ? ` — ${stat.description}` : '';
            instruction += `      {"name": "${stat.name}", "value": X}${comma}  // 0 to 100${charStatDesc}\n`;
        }
        instruction += '    ]';
    }
