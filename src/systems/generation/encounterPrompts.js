...
/**
 * Builds a summary of the player's custom stats (Energy, Satiety, etc.) for encounter prompts.
 * Returns an array of { id, name, value, maxValue, displayMode } objects for enabled stats.
 * Excludes the Health stat since HP is handled separately in combatStats.
 * @returns {Array} Array of stat descriptor objects
 */
function getCustomStatsForEncounter() {
    ...
    // Exclude health — it maps to HP which the AI already manages in combatStats
    return enabledStats
        .filter(stat => stat.id !== 'health')
        .map(stat => {
            const maxValue = stat.maxValue || 100;
            const rawValue = userStats[stat.id] !== undefined ? userStats[stat.id] : (displayMode === 'number' ? maxValue : 100);
            return {
                id: stat.id,
                name: stat.name,
                value: rawValue,
...
/**
 * Builds a human-readable string summarising non-HP custom stats for injection into prompts.
 * @returns {string} Multi-line string, or empty string if no custom stats.
 */
function buildCustomStatsContext() {
    const stats = getCustomStatsForEncounter();
    if (stats.length === 0) return '';

    const lines = stats.map(s => {
        if (s.displayMode === 'number') {
            return `${s.name}: ${s.value}/${s.maxValue}`;
        }
...
/**
 * Builds the JSON format snippet for custom stats inside the combatStats party member block.
 * @param {boolean} [useFullValues=false] - When true, emits maxValue for every stat value
 *   (used for NPC/enemy template entries). When false, emits the player's real current values.
 * @returns {string} Indented JSON lines for a "customStats" array, or empty string if no custom stats.
 */
function buildCustomStatsJSONFormat(useFullValues = false) {
    const stats = getCustomStatsForEncounter();
    if (stats.length === 0) return '';

    const entries = stats.map(s => {
        const value = useFullValues ? s.maxValue : s.value;
        return `        {"id": "${s.id}", "name": "${s.name}", "value": ${value}, "maxValue": ${s.maxValue}}`;
    });
...
/**
 * Builds a list of enabled Character Stats (from Present Characters > Character Stats config)
 * for use in non-player party member and enemy customStats blocks.
 * Returns an array of { id, name, maxValue } descriptor objects.
 * @returns {Array} Array of character stat descriptor objects
 */
function getCharacterStatsForEncounter() {
    const charStatsConfig = extensionSettings.trackerConfig?.presentCharacters?.characterStats;
    if (!charStatsConfig?.enabled) return [];

    return (charStatsConfig.customStats?.filter(s => s && s.enabled && s.name && s.id) || []).map(stat => ({
        id: stat.id,
        name: stat.name,
...
export async function buildEncounterInitPrompt() {
    ...
    try {
        ...
        if (typeof getWorldInfoFn === 'function' && currentChat && currentChat.length > 0) {
            ...
            const worldInfoResult = await getWorldInfoFn(chatForWI, 8000, false);
            const worldInfoString = worldInfoResult?.worldInfoString || worldInfoResult;
    ...
    // Add custom stats (Energy, Satiety, etc.) for encounter context
    const customStatsContext = buildCustomStatsContext();
    if (customStatsContext) {
        userStatsInfo += `${userName}'s Current Status: ${customStatsContext}\n`;
        userStatsInfo += `NOTE: These stats affect combat. Low Energy means ${userName} attacks weakly and tires quickly. Low Satiety causes sluggishness. Other stats have similar narrative consequences. Track these in the player's customStats array and update them each turn.\n\n`;
    }
    ...
    // Inject current custom stats (Energy, etc.) into the JSON format with their actual values
    const initCustomStats = getCustomStatsForEncounter();
    if (initCustomStats.length > 0) {
        initInstruction += `      "customStats": [\n`;
        initCustomStats.forEach((s, i) => {
            const comma = i < initCustomStats.length - 1 ? ',' : '';
            initInstruction += `        {"id": "${s.id}", "name": "${s.name}", "value": ${s.value}, "maxValue": ${s.maxValue}}${comma}\n`;
        });
        initInstruction += `      ],\n`;
    }
    ...
    initInstruction += `    },\n`;
    initInstruction += `    {\n`;
    initInstruction += `      "name": "PartyMemberName",\n`;
    initInstruction += `      "hp": X,\n`;
    initInstruction += `      "maxHp": X,\n`;
    // NPC party members use Character Stats (Present Characters > Character Stats config)
    const initCharStats = getCharacterStatsForEncounter();
    if (initCharStats.length > 0) {
        initInstruction += `      "customStats": [\n`;
        initCharStats.forEach((s, i) => {
            const comma = i < initCharStats.length - 1 ? ',' : '';
            initInstruction += `        {"id": "${s.id}", "name": "${s.name}", "value": ${s.maxValue}, "maxValue": ${s.maxValue}}${comma}  // set to a value fitting this character\n`;
        });
        initInstruction += `      ],\n`;
    }
    initInstruction += `      "attacks": [\n`;
    ...
    initInstruction += `      ],\n`;
    initInstruction += `      "items": [],\n`;
    initInstruction += `      "statuses": [],\n`;
    initInstruction += `      "isPlayer": false\n`;
    initInstruction += `    }\n`;
    initInstruction += `    // Include only the party members who actually exist in the context above. Remove this NPC block if no party members are present.\n`;
    initInstruction += `  ],\n`;
    ...
    if (initCharStats.length > 0) {
        ...
        initCharStats.forEach((s, i) => {
            const comma = i < initCharStats.length - 1 ? ',' : '';
            // Enemies start at full — AI will assign values appropriate to the enemy type
            initInstruction += `        {"id": "${s.id}", "name": "${s.name}", "value": ${s.maxValue}, "maxValue": ${s.maxValue}}${comma}  // set to a value fitting the enemy\n`;
        });
        initInstruction += `      ],\n`;
    }
    initInstruction += `      "attacks": [\n`;
    ...
    if (initCustomStats.length > 0 || initCharStats.length > 0) {
        initInstruction += `- customStats: The player, party members, and enemies each have a customStats array tracking non-HP stats.\n`;
        initInstruction += `  The player's values are pre-filled with their current real values. Party member and enemy values should be set to match the character's or enemy's nature.\n`;
        initInstruction += `  Keep them in the JSON and update them each turn to reflect what happened (e.g. expending energy drains Energy, resting restores it).\n`;
        initInstruction += `  These stats MUST influence the narrative and combat: if Energy is low the combatant attacks weakly or collapses; if Satiety is low they feel faint; etc.\n`;
        if (initCustomStats.length > 0) {
            const playerStatIds = initCustomStats.map(s => `"${s.name}"`).join(', ');
            initInstruction += `  CRITICAL: The player's customStats array must contain EXACTLY these stats and NO others: [${playerStatIds}]. Do NOT add, rename, or invent additional stats for the player.\n`;
        }
        if (initCharStats.length > 0) {
            const charStatIds = initCharStats.map(s => `"${s.name}"`).join(', ');
            initInstruction += `  CRITICAL: NPC party members and enemies must use EXACTLY these stats and NO others in their customStats: [${charStatIds}]. Do NOT add, rename, or invent additional stats for NPCs or enemies.\n`;
        }
        initInstruction += `\n`;
    } else {
        initInstruction += `\n`;
...
export async function buildCombatActionPrompt(action, combatStats) {
    ...
    if (context.name1) {
        ...
        // Add custom stats reminder to persona (will also appear per-turn in state message)
        const customStatsCtx = buildCustomStatsContext();
        if (customStatsCtx) {
            systemMessage += `Current Status: ${customStatsCtx}\n`;
            systemMessage += `IMPORTANT: These stats influence combat. Low Energy = weak attacks, exhaustion, may need to rest. Low Satiety = faintness, reduced effectiveness. Reflect these in the narrative every turn.\n`;
        }
    ...
    combatStats.party.forEach(member => {
        ...
        // Show custom stats if the member has them
        if (member.customStats && member.customStats.length > 0) {
            const statSummary = member.customStats.map(s => {
                const pct = s.maxValue > 0 ? Math.round((s.value / s.maxValue) * 100) : s.value;
                return `${s.name} ${s.value}/${s.maxValue}`;
            }).join(', ');
            stateMessage += `  Status: ${statSummary}\n`;
            // Add a per-turn warning for critically low stats
            member.customStats.forEach(s => {
                const pct = s.maxValue > 0 ? (s.value / s.maxValue) * 100 : s.value;
                if (pct <= 15) {
                    stateMessage += `  ⚠️ CRITICAL: ${s.name} is critically low (${s.value}/${s.maxValue})! ${member.name} is barely functioning — attacks are feeble, they may collapse.\n`;
                } else if (pct <= 30) {
                    stateMessage += `  ⚠️ LOW: ${s.name} is low (${s.value}/${s.maxValue}). ${member.name} is noticeably weakened.\n`;
                }
            });
        }
    ...
    combatStats.enemies.forEach(enemy => {
        ...
        if (enemy.customStats && enemy.customStats.length > 0) {
            const statSummary = enemy.customStats.map(s => `${s.name} ${s.value}/${s.maxValue}`).join(', ');
            stateMessage += `  Status: ${statSummary}\n`;
            enemy.customStats.forEach(s => {
                const pct = s.maxValue > 0 ? (s.value / s.maxValue) * 100 : s.value;
                if (pct <= 15) {
                    stateMessage += `  ⚠️ CRITICAL: ${enemy.name}'s ${s.name} is critically low — their attacks are feeble, they may collapse.\n`;
                } else if (pct <= 30) {
                    stateMessage += `  ⚠️ LOW: ${enemy.name}'s ${s.name} is low — they are noticeably weakened.\n`;
                }
            });
        }
    ...
    stateMessage += `    "party": [\n`;
    // Player uses user custom stats (Energy, Satiety, etc.); NPCs and enemies use character stats
    const actionCustomStats = getCustomStatsForEncounter();
    const actionCharStats = getCharacterStatsForEncounter();
    // Build lookup sets of allowed stat IDs so hallucinated stats can be stripped
    const allowedPlayerStatIds = new Set(actionCustomStats.map(s => s.id));
    const allowedCharStatIds = new Set(actionCharStats.map(s => s.id));
    combatStats.party.forEach((member, memberIdx) => {
        const isLast = memberIdx === combatStats.party.length - 1;
        stateMessage += `      {\n`;
        stateMessage += `        "name": "${member.name}",\n`;
        stateMessage += `        "hp": ${member.hp},\n`;
        stateMessage += `        "maxHp": ${member.maxHp},\n`;
        if (member.isPlayer && actionCustomStats.length > 0) {
            // Player: use the user's configured custom stats
            stateMessage += `        "customStats": [\n`;
            actionCustomStats.forEach((s, i) => {
                const comma = i < actionCustomStats.length - 1 ? ',' : '';
                // Only look up stats that are in the allowed set — ignore any extras the AI may have added
                const existingStat = member.customStats?.find(cs => cs.id === s.id && allowedPlayerStatIds.has(cs.id));
                const currentVal = existingStat !== undefined ? existingStat.value : s.value;
                stateMessage += `          {"id": "${s.id}", "name": "${s.name}", "value": ${currentVal}, "maxValue": ${s.maxValue}}${comma}\n`;
            });
            stateMessage += `        ],\n`;
        } else if (!member.isPlayer && actionCharStats.length > 0) {
            // NPC party member: use character stats, not user stats
            stateMessage += `        "customStats": [\n`;
            actionCharStats.forEach((s, i) => {
                const comma = i < actionCharStats.length - 1 ? ',' : '';
                // Only look up stats that are in the allowed set — ignore any extras the AI may have added
                const existingStat = member.customStats?.find(cs => cs.id === s.id && allowedCharStatIds.has(cs.id));
                const currentVal = existingStat !== undefined ? existingStat.value : s.maxValue;
                stateMessage += `          {"id": "${s.id}", "name": "${s.name}", "value": ${currentVal}, "maxValue": ${s.maxValue}}${comma}\n`;
            });
            stateMessage += `        ],\n`;
        }
        stateMessage += `        "statuses": ${JSON.stringify(member.statuses || [])},\n`;
        stateMessage += `        "isPlayer": ${member.isPlayer ? 'true' : 'false'}\n`;
        stateMessage += `      }${isLast ? '' : ','}\n`;
    });
    stateMessage += `    ],\n`;
    stateMessage += `    "enemies": [\n`;
    // Emit one entry per actual enemy with their real current customStats values
    combatStats.enemies.forEach((enemy, enemyIdx) => {
        const isLast = enemyIdx === combatStats.enemies.length - 1;
        stateMessage += `      {\n`;
        stateMessage += `        "name": "${enemy.name}",\n`;
        stateMessage += `        "hp": ${enemy.hp},\n`;
        stateMessage += `        "maxHp": ${enemy.maxHp},\n`;
        if (actionCharStats.length > 0) {
            // Enemies use character stats, not user stats
            stateMessage += `        "customStats": [\n`;
            actionCharStats.forEach((s, i) => {
                const comma = i < actionCharStats.length - 1 ? ',' : '';
                // Only look up stats that are in the allowed set — ignore any extras the AI may have added
                const existingStat = enemy.customStats?.find(cs => cs.id === s.id && allowedCharStatIds.has(cs.id));
                const currentVal = existingStat !== undefined ? existingStat.value : s.maxValue;
                stateMessage += `          {"id": "${s.id}", "name": "${s.name}", "value": ${currentVal}, "maxValue": ${s.maxValue}}${comma}\n`;
            });
            stateMessage += `        ],\n`;
        }
        stateMessage += `        "statuses": ${JSON.stringify(enemy.statuses || [])}\n`;
        stateMessage += `      }${isLast ? '' : ','}\n`;
    });
    stateMessage += `    ]\n`;
    stateMessage += `  },\n`;
    ...
    if (actionCustomStats.length > 0 || actionCharStats.length > 0) {
        stateMessage += `CUSTOM STATS RULES:\n`;
        if (actionCustomStats.length > 0) {
            const playerStatNames = actionCustomStats.map(s => s.name).join(', ');
            stateMessage += `- The PLAYER has their own customStats (${playerStatNames}). Update these each turn to reflect the player's actions.\n`;
            stateMessage += `- CRITICAL: The player's customStats array must contain EXACTLY these stats (${playerStatNames}) and NO others. Do NOT add, rename, or invent any additional stats for the player.\n`;
            stateMessage += `- Strenuous actions (heavy attacks, sprinting, spellcasting) drain Energy for the player. Resting restores it slightly.\n`;
            stateMessage += `- Being hit, poisoned, or bleeding may affect other player stats.\n`;
            stateMessage += `- If the player's Energy <= 15% their attacks become feeble/slow -- narrate this explicitly.\n`;
            stateMessage += `- If the player's Energy = 0 they collapse from exhaustion and cannot attack.\n`;
        }
        if (actionCharStats.length > 0) {
            const charStatNames = actionCharStats.map(s => s.name).join(', ');
            stateMessage += `- NPC party members and enemies each have their own customStats (${charStatNames}). Track them independently.\n`;
            stateMessage += `- CRITICAL: NPC and enemy customStats must contain EXACTLY these stats (${charStatNames}) and NO others. Do NOT add, rename, or invent additional stats for NPCs or enemies.\n`;
            stateMessage += `- NPC/enemy customStats use the character stat configuration, NOT the player's stats.\n`;
        }
        stateMessage += `- Never let any customStat go below 0 or above maxValue.\n`;
        stateMessage += `- Always include the full customStats array for every party member and enemy, even if unchanged.\n`;
    }
...
export async function buildCombatSummaryPrompt(combatLog, result) {
    ...
    if (extensionSettings.generationMode === 'together' && (extensionSettings.showUserStats || extensionSettings.showInfoBox || isPresentCharactersEnabled())) {
        ...
        summaryMessage += `CRITICAL: The combat encounter used an internal "customStats" array on party members (e.g. Energy, Satiety) to track stats turn-by-turn inside the fight. Do NOT copy those internal combat values directly into the tracker fields below. The tracker stats must only reflect the natural post-combat state based on the narrative outcome — derive new values from the pre-combat state and what happened in the fight, do not treat the combat engine's last customStats values as the ground truth. Specifically, do NOT introduce stat fields that do not already exist in the pre-combat tracker shown in <previous> above.\n\n`;
